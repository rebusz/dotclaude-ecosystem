"""Narrow read-only Git collector; deliberately independent of git_hygiene mutations."""

from __future__ import annotations

import hashlib
import time
from pathlib import Path

from truthdeck_collectors import CollectorError, CollectorResult, run_bounded
from truthdeck_model import CollectorRun, make_fact


def collect_git(repo: Path, *, base_ref: str, observed_at_utc: str, deadline: float) -> CollectorResult:
    started = time.monotonic()
    root = Path(_git(repo, deadline, "rev-parse", "--show-toplevel")).resolve()
    common = Path(_git(root, deadline, "rev-parse", "--path-format=absolute", "--git-common-dir")).resolve()
    head = _git(root, deadline, "rev-parse", "HEAD")
    base = _git(root, deadline, "rev-parse", base_ref)
    status = _git(root, deadline, "status", "--porcelain=v1", "--untracked-files=normal")
    merged = _git_code(root, deadline, "merge-base", "--is-ancestor", head, base) == 0
    origin = _git_optional(root, deadline, "remote", "get-url", "origin")
    repo_id = _repo_id(origin, common)
    locator = f"git:{repo_id}"
    facts = (
        make_fact("git.head", head, source_type="git", source_locator=locator,
                  observed_at_utc=observed_at_utc, repo_id=repo_id),
        make_fact("implementation.head", head, source_type="git", source_locator=locator,
                  observed_at_utc=observed_at_utc, repo_id=repo_id),
        make_fact("git.base", base, source_type="git", source_locator=locator,
                  observed_at_utc=observed_at_utc, repo_id=repo_id),
        make_fact("git.clean", not bool(status), source_type="git", source_locator=locator,
                  observed_at_utc=observed_at_utc, repo_id=repo_id),
        make_fact("git.merged", merged, source_type="git", source_locator=locator,
                  observed_at_utc=observed_at_utc, repo_id=repo_id),
    )
    return CollectorResult("git", facts, CollectorRun("git", "1", int((time.monotonic() - started) * 1000), 0, repo_id=repo_id))


def _git(repo: Path, deadline: float, *args: str) -> str:
    result = run_bounded(("git", "-C", str(repo), *args), cwd=repo, deadline=deadline)
    if result.returncode:
        raise CollectorError(f"git command failed: {' '.join(args)}: {result.stderr[:200]}")
    return result.stdout.strip()


def _git_code(repo: Path, deadline: float, *args: str) -> int:
    return run_bounded(("git", "-C", str(repo), *args), cwd=repo, deadline=deadline).returncode


def _git_optional(repo: Path, deadline: float, *args: str) -> str:
    result = run_bounded(("git", "-C", str(repo), *args), cwd=repo, deadline=deadline)
    return result.stdout.strip() if result.returncode == 0 else ""


def _repo_id(origin: str, common: Path) -> str:
    normalized = origin.replace("\\", "/").removesuffix(".git")
    if normalized:
        tail = normalized.split(":", 1)[-1].split("github.com/", 1)[-1].strip("/")
        if "/" in tail:
            return tail.lower()
    return f"local-{hashlib.sha256(str(common).lower().encode()).hexdigest()[:16]}"
