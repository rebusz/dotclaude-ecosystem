...
def print_report(r: Report, apply: bool) -> None:
    ...
    if not apply:
        print("\n(DRY-RUN — nothing changed. Re-run with --apply to reap the REAP/PRUNE sets.)")
...
def main(argv: list[str]) -> int:
    ...
    if a.apply:
        # Re-confirm no primary R3 alarm before mutating anything destructive.
        if r.primary.get("r3"):
            print("\nREFUSING --apply: PRIMARY has unpushed R3 work. Resolve that first.", file=sys.stderr)
            return 2
        print("\n=== APPLYING ===")
        do_apply(a.repo, r)
        print("done.")
