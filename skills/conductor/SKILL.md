...
4. **Pytest-Only Host Gate:** Only heavy pytest runs request the capacity-one `host:heavy` lease. CDP uses its independent `cdp:chatgpt`, `cdp:perplexity`, `cdp:gemini` or `cdp:tv` pool; host-heavy occupancy or recovery never blocks CDP. Playwright/browser work, builds, replays, docs, Git and readback are not heavy tasks. Focused pytest keeps its existing lightweight classification. A pytest child may inherit one active attempt-scoped `TDCONDUCTOR_LEASE_ID`; ownership and recovery rules apply within the requested resource pool.
