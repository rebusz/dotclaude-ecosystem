...
- `python scripts/conductorctl.py resource-live`: Read a live gate projection (pool, live state counts, holder, queue, recovery fences) in ~10 ms without writing a receipt or creating directories. Default key is `host:heavy`; pass `--all` for every pool including `cdp:*`.
- `python scripts/conductor_gui.py`: Open the read-only Conductor Gate Panel (local Tk window, port-free, signature-gated live view of all resource pools).
- `python scripts/conductorctl.py resource-status --json`: Read the durable pool, queue, recovery state, and storage growth.
...
4. **Named Resource Pools:** Heavy pytest, Playwright, stress, and costly local build/replay request Conductor's durable `host:heavy` lease (capacity one) before launch. CDP-provider work requests the matching `cdp:chatgpt` / `cdp:perplexity` / `cdp:gemini` / `cdp:tv` pool with a chrome_* role and model/profile lock — never `host:heavy`. Plan, docs, Git, and focused pytest do not occupy heavy. A child may inherit one active attempt-scoped `TDCONDUCTOR_LEASE_ID`; a second inherited child, forged/stale token, or overlapping independent request is refused or queued. An `ERROR` receipt is a failed command (non-zero process exit).
