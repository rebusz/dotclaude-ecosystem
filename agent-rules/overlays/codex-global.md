...
- Codex passes `--synthesizer gpt`; `--synthesizer gpt|claude` records the active final judge and never changes the panel. Every client/risk class runs ChatGPT CDP (only GPT-5.6 Sol: Pro effort, then safe pre-submit xhigh fallback on the same model), Antigravity `gemini-3.7-flash-high` (Gemini CDP `gemini-3.7-flash` fallback), and Perplexity GLM 5.3/Kimi 3/Grok 4.6/Sonnet 5/GPT Terra. Claude CLI, Codex CLI, standalone GLM CLI, and nested CLI tournament synthesis are not workflow lanes.
