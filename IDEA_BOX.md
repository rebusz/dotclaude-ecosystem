...
- [P1][M] **Conductor orphan auto-recovery** — operator 2026-08-31 odrzucił ręczne-only `OWNER_LIVENESS_UNPROVEN`: owner identity ma być trwałe, właściciel z udowodnioną śmiercią sprzątany automatycznie przed admission/promocją, a niejednoznaczny nadal fail-closed. Implementacja: `design/handoffs/2026-08-31_conductor_orphan_auto_recovery_r2_implementation_handoff.md`; bramka `GO CONDUCTOR ORPHAN AUTO-RECOVERY R2`.
- [P2][M] **Wspólne GUI: Conductor Gate Panel + EcosystemControl** — dziś operator ma dwa osobne okna na jeden stan hosta: Gate Panel (`scripts/conductor_gui.py`, Tk, read-only, wjechał w #89) i `D:/APPS/EcosystemControl`. Scalić w jeden panel, żeby bramki `host:heavy` / pule `cdp:*` / recovery były widoczne obok reszty ekosystemu, zamiast wymagać zgadywania, które okno mówi prawdę. Trigger: 2026-08-29 — operator nie wiedział, gdzie w ogóle jest Conductor GUI, mimo że było zmergowane. Warunek brzegowy: panel zostaje **read-only** projekcją; scalenie UI nie może dać EcosystemControl drugiej władzy nad admission (to samo ograniczenie co w planie R1 wyżej). Do rozstrzygnięcia przed planem: który proces jest gospodarzem okna i czy Tk wystarczy, czy EcosystemControl ma własny stack UI.
