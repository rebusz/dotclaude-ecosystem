...
2. **Superseded audit aliases** (auditq/auditai/auditpx) — **INSIDE the managed block** (`~/.claude/CLAUDE.md` lines 134–136), sourced from **`agent-rules/overlays/claude-global.md:14–16`** → **edit the overlay SOURCE** (collapse to a one-line pointer), CREATE `~/.claude/refs/audit-aliases.md` with the table, then `sync --write`. Editing the rendered CLAUDE.md would be silently clobbered.
3. **`core.md` surgery** — edit **`agent-rules/core.md` SOURCE**: extract the deep procedure from `Token Budget Protocol` (8 bullets) + the `Trading data flow` seam into `agent-rules/refs/token-budget-protocol.md` + `refs/data-flow-seam.md` (CREATE `agent-rules/refs/` first), leaving a 2-line invariant + pointer in `core.md`; then `sync_agent_rules.py --write`. **BLAST RADIUS (eng-review):** re-sync regenerates **~28 managed-block files** (13 repos × {CLAUDE.md, AGENTS.md} + 2 global) — the merge diff expects that wide-but-mechanical fan-out. ("≤8 files" = hand-edits only.)
4. **MEMORY.md hot/cold cleanup** — edit `MEMORY.md` (53 entries / 15,618 B) → move DONE/superseded entries (resolved blockers, merged PRs) to its **existing** `ARCHIVE.md` per the operator's own rule (core.md line 34). Target: index back under its load cap.
5. **Fail-CLOSED guard (amendment #3, CORRECTED by eng-review):** the slimmed CLAUDE.md kernel is today guarded ONLY by `line_limit=200` on the **claude-global TargetSpec** (`sync_agent_rules.py` ~line 216; 178/200 = 22 lines headroom) and has **NO byte_limit**; the 32 KB byte_limits (~lines 221/296) guard the **codex AGENTS.md** targets, not CLAUDE.md. **Fix: tighten `line_limit` AND ADD an explicit `byte_limit` (~8–9 KB, just above the ~7 KB slim) on the claude-global spec.** Leave the codex/repo 32 KB constants alone. Without this, regrowth fails OPEN.
...
### Accepted expansions (operator GO 2026-06-27 — fold before implementation)
1. **Standing cost-regression CLI** — `scripts/session_cost_probe.py` emits a tracked baseline JSON, wrapping the §9 B0/A1/A2 method; every future kernel/skill/MCP change must clear the cost gate (the durable home of §9). [S] **This becomes the measurement vehicle for Slice 0's A1 metric.**
